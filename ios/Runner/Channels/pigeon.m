...
+(ChannelResponse*)fromMap:(NSDictionary*)dict {
  ...
  result.request = [ChannelRequest fromMap:dict[@"request"]];
...
-(NSDictionary*)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.status ? self.status : [NSNull null]), @"status", (self.result ? self.result : [NSNull null]), @"result", (self.error ? [self.error toMap] : [NSNull null]), @"error", (self.request ? [self.request toMap] : [NSNull null]), @"request", nil];
}
@end
...
void AndroidLifecycleHelperSetup(id<FlutterBinaryMessenger> binaryMessenger, id<AndroidLifecycleHelper> api) {
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        messageChannelWithName:@"dev.flutter.pigeon.AndroidLifecycleHelper.sendToBackground"
        binaryMessenger:binaryMessenger];
    if (api) {
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
        [api sendToBackground:&error];
